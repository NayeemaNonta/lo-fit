...
class LlamaAttention(nn.Module):
    ...
    use_cache: bool = False,
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = torch.cat([F.linear(hidden_states, w) for w in query_slices], dim=-1)
            key_states   = torch.cat([F.linear(hidden_states, w) for w in key_slices], dim=-1)
            value_states = torch.cat([F.linear(hidden_states, w) for w in value_slices], dim=-1)
        else:
        ...
        # 0) project
        query_states = self.q_proj(hidden_states)   # [B, T, H*D]
        key_states   = self.k_proj(hidden_states)   # [B, T, HK*D]
        value_states = self.v_proj(hidden_states)   # [B, T, HK*D]

        # 1) reshape to [B, H, T, D] and [B, HK, T, D]
        ...
        # 2) concat past FIRST (if any), along sequence dimension
        if past_key_value is not None and past_key_value[0] is not None:
            key_states   = torch.cat([past_key_value[0], key_states], dim=2)   # dim=2 is seq
            value_states = torch.cat([past_key_value[1], value_states], dim=2)

        # 3) kv length AFTER concat
        kv_seq_len = key_states.shape[-2]

        # ensure RoPE cache is long enough for positions
        if position_ids is not None:
            kv_seq_len = max(kv_seq_len, int(position_ids.max().item()) + 1)

        # 4) apply RoPE to q/k using that kv_seq_len
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)  # OK to use value_states for device/dtype
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids)

        # 5) set cache (use the NON-repeated k/v for cache)
        present_key_value = (key_states, value_states) if use_cache else None

        # 6) GQA fan-out (doesn't change seq_len)
        ...
        # (optional) LoFiT per-head affine
        if self.applied_module == 'attention':
            for i, (A, v) in enumerate(zip(self.attn_A, self.attn_v)):
                s = slice(i * self.head_dim, (i + 1) * self.head_dim)
                attn_output[:, :, s] = (A + 1) * attn_output[:, :, s].clone() + v
