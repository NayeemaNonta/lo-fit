...
# coding=utf-8
# LoFiT-friendly LLaMA modeling (PyTorch)
# - Adds LoFiT parameters (attn_A, attn_v) on attention heads
# - Normalizes rope_scaling to support both Llama-2 and Llama-3 configs
# - Exposes LlamaForCausalLM.custom_from_pretrained for training scripts
...
def _normalize_rope_scaling(cfg: LlamaConfig) -> Optional[Dict[str, Any]]:
    """
    ...
    if isinstance(rs, dict) and "type" in rs:
        # Already the Llama-2 format
        return rs

    if isinstance(rs, dict) and ("rope_type" in rs or "factor" in rs):
        factor = rs.get("factor", 1.0)
        normalized = {"type": rs.get("rope_type", "llama3"), "factor": factor}
        # Preserve Llama-3 keys in case downstream code wants them
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """Rotary Embedding with linear scaling."""
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        self.max_seq_len_cached = seq_len
        t = torch.arange(self.max_seq_len_cached, device=device, dtype=self.inv_freq.dtype) / self.scaling_factor
        freqs = torch.einsum("i,j->ij", t, self.inv_freq)
        emb = torch.cat((freqs, freqs), dim=-1)
        self.register_buffer("cos_cached", emb.cos()[None, None, :, :].to(dtype), persistent=False)
        self.register_buffer("sin_cached", emb.sin()[None, None, :, :].to(dtype), persistent=False)
...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """Rotary Embedding with Dynamic NTK scaling."""
...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            gate_proj = torch.cat([F.linear(x, s) for s in gate_proj_slices], dim=-1)
            up_proj = torch.cat([F.linear(x, s) for s in up_proj_slices], dim=-1)
...
# -------------------------
# Core modules with LoFiT
# -------------------------

class LlamaAttention(nn.Module):
    """Multi-headed attention with LoFiT head-wise offsets."""
    ...
    def _init_rope(self):
        rs = _normalize_rope_scaling(self.config)
        if rs is None:
            self.rotary_emb = LlamaRotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings)
            return
        ...
        if scaling_type == "linear":
            self.rotary_emb = LlamaLinearScalingRotaryEmbedding(
                self.head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=factor
            )
        elif scaling_type == "dynamic":
            self.rotary_emb = LlamaDynamicNTKScalingRotaryEmbedding(
                self.head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=factor
            ...
            # Treat Llama-3 rope as linear scaling with its factor
            self.rotary_emb = LlamaLinearScalingRotaryEmbedding(
                self.head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=factor
            )
        else:
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = [F.linear(hidden_states, s) for s in query_slices]
            query_states = torch.cat(query_states, dim=-1)

            key_states = [F.linear(hidden_states, s) for s in key_slices]
            key_states = torch.cat(key_states, dim=-1)

            value_states = [F.linear(hidden_states, s) for s in value_slices]
            value_states = torch.cat(value_states, dim=-1)
        else:
            query_states = self.q_proj(hidden_states)
            key_states = self.k_proj(hidden_states)
        ...
        if self.applied_module == "attention":
            for i, (A, v) in enumerate(zip(self.attn_A, self.attn_v)):
                ...
                attn_output[:, :, start:end] = torch.mul(A + 1, attn_output[:, :, start:end].clone()) + v

        # Output projection
        if self.config.pretraining_tp > 1:
            out_slices = self.o_proj.weight.split(self.hidden_size // self.config.pretraining_tp, dim=1)
            attn_output = attn_output.split(self.hidden_size // self.config.pretraining_tp, dim=2)
            attn_output = sum([F.linear(attn_output[i], out_slices[i]) for i in range(self.config.pretraining_tp)])
        else:
            attn_output = self.o_proj(attn_output)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def set_applied_modules_to_layers(self, applied_module: str, applied_layers: Optional[List[int]] = None):
        """
        Mark which layers receive LoFiT interventions (on attention output).
        """
        if applied_layers is None:
            applied_layers = list(range(len(self.layers)))
        for idx in applied_layers:
    ...
    # Copied from transformers.models.bart.modeling_bart.BartDecoder._prepare_decoder_attention_mask
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        # create causal mask: [bsz, 1, tgt_len, src_len]
        combined_attention_mask = None
        if input_shape[-1] > 1:
            combined_attention_mask = _make_causal_mask(
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, pkv, output_attentions)
                    return custom_forward
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:
            slice_size = self.vocab_size // self.config.pretraining_tp
            lm_head_slices = self.lm_head.weight.split(slice_size, dim=0)
            logits = [F.linear(hidden_states, s) for s in lm_head_slices]
            logits = torch.cat(logits, dim=-1)
        else:
            logits = self.lm_head(hidden_states)
...
# Public exports
__all__ = ["LlamaModel", "LlamaForCausalLM", "LlamaForSequenceClassification"]
